...
export function Sidebar() {
  ...
  return (
    /* 
      ...
      - Thin 3px strip always visible on left edge
      - Full sidebar slides in on hover of this wrapper
    */
    <div className="group fixed left-0 top-0 h-screen z-50">
      {/* Hover trigger strip — always visible, shows the sidebar is there */}
      <div className="absolute left-0 top-0 w-3 h-full bg-gradient-to-b from-cmtx-blue via-cmtx-navy to-slate-900 opacity-80 group-hover:opacity-0 transition-opacity duration-300 pointer-events-none" />

      {/* Sidebar panel — slides in on hover */}
      <div className="w-64 h-full bg-cmtx-navy text-white flex flex-col -translate-x-full group-hover:translate-x-0 transition-transform duration-300 ease-in-out shadow-2xl border-r border-white/5">
        {/* Logo */}
                  ...
                  return (
                    <div key={item.name}>
                      <Link
                        href={item.href}
                        className={cn(
                          "flex items-center justify-between px-4 py-3 rounded-xl transition-all duration-300 group/link text-sm font-semibold relative overflow-hidden",
                          isActive
                      ...
                      >
                        ...
                        <div className="flex items-center gap-3">
                          <item.icon className={cn("w-5 h-5 transition-transform duration-300", isActive ? "scale-110" : "group-hover/link:scale-110")} />
                        ...
                        {item.subItems ? (
                          ...
                          isExactActive && <ChevronRight className="w-4 h-4 opacity-50 shrink-0" />
                        )}
                      </Link>
                ...
                {supportItems.map((item) => {
                  ...
                  return (
                    <Link
                      ...
                      href={item.href}
                      className={cn(
                        "flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-300 group/link text-sm font-semibold",
